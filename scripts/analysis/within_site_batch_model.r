...
# Land cover
landcover <- rast("lrgv_dynamic_world_mode_2015_2025.tif")
...
for (i in 1:nrow(viable)) {
  ...
  tryCatch({
    ...
    # All observations at site (for effort)
    ...
    # Filter to cells with effort
    hex_effort <- hex_filtered %>% filter(cell_id %in% cells_with_effort)
    # Renumber cells 1:n for modeling
    cell_lookup <- data.frame(
      old_id = hex_effort$cell_id,
      new_id = 1:nrow(hex_effort)
    )
    hex_effort$model_cell_id <- 1:nrow(hex_effort)
    
    cat("  Cells with effort:", n_cells_effort, "\n")
    
    ####################
    #   Extract Covariates
    ####################
    
    cov_data <- data.frame(cell_id = hex_effort$model_cell_id)
    ...
    # Select only viable covariates
    # Map viable_covs names to cov_data names
    cov_name_map <- c(
      "trees" = "trees",
      "grass" = "grass", 
      "shrub" = "shrub",
    ...
    # Check variance and remove any zero-variance
    ...
    # Map observations to model cell IDs
    sp_sf$model_cell_id <- cell_lookup$new_id[match(sp_sf$cell_id, cell_lookup$old_id)]
    sp_df <- st_drop_geometry(sp_sf)
    
    for (k in 1:nrow(sp_df)) {
      obs <- sp_df[k, ]
      
      cell_idx <- obs$model_cell_id
    ...
    # Clean up
    rm(model, detection_matrix, data_list, hex_effort, sp_sf, all_sf)
    gc(verbose = FALSE)
...
print(summary_results %>%
        ...
        summarise(
          n_models = n(),
          n_converged = sum(model_converged),
          n_ppc_pass = sum(ppc_pass),
          mean_naive_occ = round(mean(naive_occ), 3)
        ))
