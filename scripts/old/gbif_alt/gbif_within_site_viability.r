##############################
#
# Within-Site Model Viability Assessment — GBIF Data
# Ian Becker
# May 2026
#
##############################
...
cat("=== WITHIN-SITE VIABILITY ASSESSMENT — GBIF ===\n")
...
# Sites — filter to 61 GBIF sites
sites_all <- st_read("lrgv_green_spaces_detection_filtered", quiet = TRUE)
site_covs <- readRDS("site_covariates_gbif.rds")
gbif_site_ids <- sort(unique(site_covs$site_id))

sites     <- sites_all %>% filter(site_id %in% gbif_site_ids)
sites_utm <- st_transform(sites, crs = 32614)

cat("Loaded", nrow(sites), "GBIF sites\n")
...
cat("Loaded", nrow(gbif), "GBIF observations after year filter\n")
...
gbif_study <- gbif %>% filter(common_name %in% study_species)

cat("Observations of study species:", nrow(gbif_study), "\n\n")
...
candidate_sites <- unique(gbif_study$site_id)
...
for (sid in candidate_sites) {
  ...
  for (sp in site_species) {
    ...
    sp_obs <- gbif_study %>% filter(common_name == sp, site_id == sid)
...
write.csv(viability_results,
          "within_site_viability_assessment_gbif.csv", row.names = FALSE)
cat("Saved: within_site_viability_assessment_gbif.csv\n")
...
write.csv(species_summary,
          "within_site_species_summary_gbif.csv", row.names = FALSE)
